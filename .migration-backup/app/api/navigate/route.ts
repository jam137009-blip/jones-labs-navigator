...
import { isInterpretation, isNavigatorResult } from "../../navigator";
...
async function callOpenAI(apiKey: string, stage: "interpret" | "result", input: Record<string, unknown>, requestId: string) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}`, "X-Client-Request-Id": requestId },
        body: JSON.stringify({
          model: "gpt-5.6", store: false, reasoning: { effort: "medium" }, max_output_tokens: stage === "interpret" ? 2000 : 3500,
          instructions: COUNCIL_INSTRUCTIONS,
          input: JSON.stringify({ stage, userData: input }),
          text: { format: { type: "json_schema", name: stage === "interpret" ? "navigator_interpretation" : "navigator_result", strict: true, schema } },
        }),
      });
...
export async function POST(request: Request) {
  ...
  let status = 500;
  try {
    if (!allowRequest(getIp(request))) { status = 429; return NextResponse.json({ error: "Navigator is receiving many requests. Please wait a moment and try again.", requestId }, { status }); }
    ...
    if (!stage || intention.length < 12 || intention.length > 4000) { status = 400; return NextResponse.json({ error: "Please describe your intention in a little more detail.", requestId }, { status }); }
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) { status = 503; return NextResponse.json({ error: "Navigator intelligence is not configured yet. Your local draft is still safe.", requestId }, { status }); }
    ...
    status = 200;
    return NextResponse.json({ data: value, requestId });
  } catch (error) {
    const timedOut = error instanceof Error && error.name === "AbortError";
    status = timedOut ? 504 : 502;
    return NextResponse.json({ error: timedOut ? "Navigator took too long to respond. Please try again." : "Navigator could not complete this step. Please try again.", requestId }, { status });
  } finally {
    console.info(JSON.stringify({ event: "navigator_request", requestId, status, latencyMs: Date.now() - startedAt, timestamp: new Date().toISOString() }));
  }
}
